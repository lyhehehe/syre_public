...
for ii=1:populationSize
    for nvar=1:numVariables
        Parent(ii,nvar) = Initial(nvar,1)+(Initial(nvar,2) - Initial(nvar,1))*rand();
    end
end
...
for n=1:generations
    ...
    for ii=1:populationSize
        ...
        Mutant(ii,:)= Parent(rev(1),:)+sf*(Parent(rev(2),:)-Parent(rev(3),:));

        for nvar=1:numVariables %Bounds are always verified
            if Mutant(ii,nvar)<Bounds(nvar,1)
                Mutant(ii,nvar) = Bounds(nvar,1);
            elseif Mutant(ii,nvar)>Bounds(nvar,2)
                Mutant(ii,nvar)=Bounds(nvar,1);
            end
        end
        ...
        for nvar=1:numVariables
            if rand > crossOverRate
                Child(ii,nvar) = Parent(ii,nvar);
            else
                Child(ii,nvar) = Mutant(ii,nvar);
            end
        end
...
if numel(s)<3
    ...
    for ii=1:s(3)
        M1=[M1;OUT.MatrixPset{ii}]; %#ok<AGROW>
